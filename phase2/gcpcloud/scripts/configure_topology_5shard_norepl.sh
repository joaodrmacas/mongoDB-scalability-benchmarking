...
sudo kubectl exec -it "$MONGO_SHARD4_POD" -- mongosh --eval 'rs.initiate({ _id: "shard4ReplSet", members: [{ _id: 0, host: "mongo-shard4-0.mongo-shard4:27017" }] })'

echo "Configuring shard 5..."
sudo kubectl exec -it "$MONGO_SHARD5_POD" -- mongosh --eval 'rs.initiate({ _id: "shard5ReplSet", members: [{ _id: 0, host: "mongo-shard5-0.mongo-shard5:27017" }] })'
...
echo "Configuring the topology on $MONGO_MONGOS2_POD..."
